...
The Desktop bind keeps native-provider capabilities available without exposing a general-purpose OAuth proxy. Responses, model discovery, and thread goals use the managed account pool; when the current upstream does not expose thread-goal endpoints, the router keeps those goals in its bounded process-local fallback. Exact secret-path requests for native image generation, image editing, and hosted search pass through with the Desktop client's native ChatGPT credentials. The loopback router rejects these native routes when callers use only the legacy local token or an incorrect route secret.
