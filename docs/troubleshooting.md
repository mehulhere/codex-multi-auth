...
| `/status` has quota rows but no account row | The Desktop build does not include the optional Linux status feature, or the router has not recorded a successful response for this thread | Enable `multi-auth-thread-status`, rebuild/reinstall Desktop, run `codex-multi-auth rotation bind-app`, then send one message in the thread |
| One Desktop thread returns `codex_pinned_account_unavailable` while others work | An older app router is still honoring a global `switch` pin whose account is cooling down, circuit-open, disabled, or rate-limited | Run `codex-multi-auth unpin` for immediate recovery, update to a build where app-bind uses automatic routing, then run `codex-multi-auth rotation reset-runtime` |
| Existing Desktop tasks reconnect to an old localhost port after `reset-runtime`, while new tasks work | An older reset implementation unbound and rebound the app, changing the router port and bearer token that existing tasks had persisted | Update codex-multi-auth, then keep a compatibility router on the retired endpoint until those tasks are migrated. Current `reset-runtime` restarts in place and preserves endpoint identity |
