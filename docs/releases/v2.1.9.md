...
Patch release for issue #477. `codex-multi-auth login --device-auth` now
completes end-to-end on Windows and Linux instead of crashing on launch,
and explicit transport flags (`--device-auth`, `--manual`, `--no-browser`)
no longer get swallowed by the Accounts Dashboard when accounts already
exist.
